...
<body class="bg-slate-50 min-h-screen flex">
    <aside class="w-64 bg-slate-900 text-white flex flex-col hidden md:flex">
        ...
        <nav class="flex-1 px-4 space-y-2">
            ...
            <a href="<?php echo BASE_URL; ?>/admin/employees" class="flex items-center px-4 py-3 bg-indigo-600 rounded-xl">Employees</a>
            <a href="<?php echo BASE_URL; ?>/admin/customers" class="flex items-center px-4 py-3 hover:bg-slate-800 rounded-xl transition">Customers</a>
    ...
    <main class="flex-1 p-8">
        ...
        <div class="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm mb-8">
            ...
            <form action="<?php echo BASE_URL; ?>/admin/save-employee" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <input type="hidden" name="employee_id" value="<?php echo $edit_emp['id'] ?? ''; ?>">
                <input type="hidden" id="emp_existing_image" name="existing_image" value="<?php echo $edit_emp['image_url'] ?? ''; ?>">
                <div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="file" name="image" onchange="previewLocalFile(this,'emp_image_preview')" class="w-full px-4 py-2 rounded-xl border border-slate-200 outline-none">
                        <button type="button" onclick="openAssetPicker('emp_existing_image','emp_image_preview')" class="whitespace-nowrap px-4 py-2 rounded-xl border border-indigo-200 text-indigo-600 font-semibold text-sm hover:bg-indigo-50 transition">Browse Assets</button>
                    </div>
                    <img id="emp_image_preview" src="<?php echo !empty($edit_emp['image_url']) ? asset_url($edit_emp['image_url']) : ''; ?>" class="mt-2 h-16 rounded-lg object-cover border border-slate-100 <?php echo empty($edit_emp['image_url']) ? 'hidden' : ''; ?>" alt="">
                </div>
                <div class="md:col-span-2 lg:col-span-3 flex gap-4">
                    ...
                    <?php if($edit_emp): ?>
                        <a href="<?php echo BASE_URL; ?>/admin/employees" class="bg-slate-200 text-slate-700 px-8 py-3 rounded-xl font-bold">Cancel</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        ...
                        <td class="px-6 py-4 text-right">
                            <a href="<?php echo BASE_URL; ?>/admin/employees?edit=<?php echo $emp['id']; ?>" class="text-indigo-600 hover:text-indigo-800 font-bold mr-4">Edit</a>
                            <a href="<?php echo BASE_URL; ?>/admin/delete-employee?id=<?php echo $emp['id']; ?>" onclick="return confirm('Remove employee?')" class="text-red-500 hover:text-red-700 font-bold">Delete</a>
                        </td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
